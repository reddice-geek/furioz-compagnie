...
function initNav(){
  ...
  links.forEach(l=>{ l.addEventListener('click', e=>{ const href = l.getAttribute('href'); if(!href.startsWith('#')) return; e.preventDefault(); const target = document.querySelector(href); if(target) window.scrollTo({top: target.offsetTop - 72, behavior:'smooth'}); }); });
...
function initModals() {
  ...
  function openMember(id) {
    ...
    if (!m) {
      ...
      modal.classList.add('active'); document.body.style.overflow="hidden"; return;
    }
    content.innerHTML = `
      <div style="display:flex; justify-content:space-between; align-items:start; gap:1rem">
        <div>
          <h2 style="margin:0 0 .2rem">${m.name} ${m.role === 'FONDATEUR • 🇫🇷 France' ? '<span style="color:var(--blue); font-weight:400; font-size:.9rem">• FONDATEUR</span>' : `<span style="color:var(--blue); font-weight:400; font-size:.85rem">• ${m.role}</span>`}</h2>
    ...
    const closeBtn = document.getElementById('close-modal'); if(closeBtn) closeBtn.onclick = () => closeModal();
  }
  function closeModal(){ modal.classList.remove('active'); document.body.style.overflow=""; }
  grid.addEventListener('click', (e)=>{ const card = e.target.closest('.team-card[data-id]'); if(!card) return; openMember(card.getAttribute('data-id')); });
