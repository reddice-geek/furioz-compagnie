...
async function seedGuestbook(redis) {
  ...
  // IMPORTANT :
  // on sème les avis par défaut UNE SEULE FOIS.
  // Sinon, si l'admin supprime tous les avis,
  // ils réapparaissent au prochain GET.
  const seededKey =
    "furioz:guestbook:seeded:v1";

  const alreadySeeded =
    await redis.get(seededKey);
  ...
  // Marqueur permanent :
  // même si tous les avis sont ensuite supprimés,
  // ils ne seront jamais recréés automatiquement.
  multi.set(
    seededKey,
    "1"
  );
